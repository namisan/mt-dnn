...
from msrlongt5.configuration import MSRLongT5Config
# local attention/s4
# from module.local_attention import LocalAttention
from module.s4 import S4Module

logger = logging.get_logger(__name__)

_CONFIG_FOR_DOC = "MSRLongT5Config"
_TOKENIZER_FOR_DOC = "MSRLongT5Tokenizer"
_CHECKPOINT_FOR_DOC = "MSRLongT5-small"
...
class T5DenseActDense(nn.Module):
    def __init__(self, config: MSRLongT5Config):
        super().__init__()
        have_bias = True #getattr(config, "have_bias", False)
...
class T5LocalAttention(nn.Module):
    ...
    def compute_bias_local_attention(self, device: torch.device):
        ...
        relative_position_bucket = self.relative_position_bucket_local.to(device)
...
class T5Attention(nn.Module):
    def __init__(self, config: MSRLongT5Config, has_relative_attention_bias=False):
        ...
        self.gradient_checkpointing = False
        relative_position = (
            torch.arange(config.max_position_embeddings, dtype=torch.long)[None, :]
            - torch.arange(config.max_position_embeddings, dtype=torch.long)[:, None]
        )
...
class T5Block(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False, layer_idx=-1):
        super().__init__()
        #long =5, decoder=3, encoder=2
        self.is_decoder = config.is_decoder
        ...
        elif layer_idx > -1 and layer_idx % config.s4_every_n_layers == 0:
            self.layer.append(T5LayerLocalGlobalSelfAttention(config, has_relative_attention_bias=has_relative_attention_bias))
        else:
            self.layer.append(T5LayerLocalSelfAttention(config, has_relative_attention_bias=has_relative_attention_bias))
    ...
    def forward(
        ...
        if self.long_seq:
            self.norm_local(hidden_states)
            hidden_states = torch.concat((global_hidden_states, hidden_states), dim=-1)
            hidden_states = self.aggregate(hidden_states)
...
class MSRLongT5PreTrainedModel(PreTrainedModel):
    """
    An abstract class to handle weights initialization and a simple interface for downloading and loading pretrained
    models.
    """

    config_class = MSRLongT5Config
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, T5LayerNorm):
        ...
        elif isinstance(module, (MSRLongT5Model, MSRLongT5ForConditionalGeneration, MSRLongT5EncoderModel)):
            ...
            if hasattr(module.wo, "bias") and module.wo.bias is not None:
                module.wo.bias.data.zero_()
        elif isinstance(module, T5LocalAttention):
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, (T5LocalAttention, T5Stack)):
...
class MSRLongT5Stack(MSRLongT5PreTrainedModel):
    def __init__(self, config, embed_tokens=None):
        ...
        self.first_layer_norm = torch.nn.LayerNorm(config.d_model, eps=config.layer_norm_epsilon, elementwise_affine=True)

        if not self.is_decoder:
            self.block = nn.ModuleList(
                [T5Block(config, has_relative_attention_bias=True, layer_idx=i) for i in range(config.num_layers)]
            )
        else:
            self.block = nn.ModuleList(
    ...
    def deparallelize(self):
        ...
        if self.is_decoder:
            self.pos_embed = self.self.pos_embed.to("cpu")
...
class MSRLongT5Model(MSRLongT5PreTrainedModel):
    ...
    def __init__(self, config: MSRLongT5Config):
        ...
        encoder_config.have_bias = False
        encoder_config.use_cache = False
        encoder_config.is_encoder_decoder = False
        self.encoder = MSRLongT5Stack(encoder_config, self.shared)

        decoder_config = copy.deepcopy(config)
        decoder_config.is_decoder = True
        decoder_config.is_encoder_decoder = False
        encoder_config.have_bias = True
        decoder_config.num_layers = config.num_decoder_layers
        self.decoder = MSRLongT5Stack(decoder_config, self.shared)
...
class MSRLongT5ForConditionalGeneration(MSRLongT5PreTrainedModel):
    _keys_to_ignore_on_load_missing = [
        r"encoder.embed_tokens.weight",
        r"decoder.embed_tokens.weight",
        r"lm_head.weight",
    ...
    def __init__(self, config: MSRLongT5Config):
        ...
        decoder_config.num_layers = config.num_decoder_layers
        self.decoder = MSRLongT5Stack(decoder_config, self.shared)
        if config.decoder_roberta_lm_head:
            self.lm_head = RobertaLMHead(config.d_model, config.vocab_size,)
        else:
...
class MSRLongT5EncoderModel(MSRLongT5PreTrainedModel):
