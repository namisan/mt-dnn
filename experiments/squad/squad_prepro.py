...
def search_index(input_ids, sequence_ids, offsets, cls_index, start_char, end_char, pad_on_right=False):
                ...
                if len(answers) == 0 or len(answers["answer_start"]) == 0:
                    ...
                    if type(start_char) is list:
                        ...
                        for sc in start_char:
                            ...
                            sp, ep = search_index(input_ids, sequence_ids, offsets, cls_index, sc, end_char, pad_on_right=pad_on_right)
                        ...
                        start_position, end_position = search_index(input_ids, sequence_ids, offsets, cls_index, start_char, end_char, pad_on_right=pad_on_right)
                        tokenized_examples["start_positions"].append(start_position)
                        tokenized_examples["end_positions"].append(end_position)
...
def prepare_validation_features(tokenizer, samples, output_path, max_seq_length=384, doc_stride=128, pad_on_right=True, pad_to_max_length=True, label_mapper=None):
    # Tokenize our examples with truncation and maybe padding, but keep the overflows using a stride. This results
    # in one example possible giving several features when a context is long, each of those features having a
    # context that overlaps a bit the context of the previous feature.
    if not tokenizer.cls_token:
...
def parse_args():
    ...
    parser.add_argument('--task_def', type=str, default="experiments/squad/squad_task_def.yml")
...
def main(args):
    ...
    for task in task_defs.get_task_names():
        ...
        for split_name in task_def.split_names:
            ...
            if is_training:
                prepare_train_feature(tokenizer, rows, dump_path, pad_on_right=pad_on_right, label_mapper=task_def.label_vocab, max_seq_length=args.max_seq_length, doc_stride=args.doc_stride)
            else:
                prepare_validation_features(tokenizer, rows, dump_path, pad_on_right=pad_on_right, label_mapper=task_def.label_vocab, max_seq_length=args.max_seq_length, doc_stride=args.doc_stride)
