...
class ScalarMix(torch.nn.Module):
    ...
    def __init__(self, mixture_size, do_layer_norm=False):
        ...
        self.scalar_parameters = ParameterList([Parameter(torch.FloatTensor([1.0/self.mixture_size]))
                                                for _ in range(mixture_size)])
        self.gamma = Parameter(torch.FloatTensor([1.0]))

    def l2norm(self):
        loss=0
        for para in self.scalar_parameters:
    ...
    def forward(self, tensors, mask=None):
        ...
        if not self.do_layer_norm:
            ...
            final_rep = self.gamma * sum(pieces)
